...
async def record_space(user_id: str, platform_id: int, label: str | None, space_user_id: str) -> None:
    """Note that this identity manages this server.

    Access, never ownership. The server keeps its own account and its own
    sources; this only says who may see them from the app.

    These rows are advisory for Discord. The live MANAGE_GUILD check when
    the dashboard loads is the authority, so losing the permission removes
    access on the next load rather than whenever somebody remembers.
    """
    async with _client() as client:
        ...
        found = await client.get(
            "/uwufeed_identities",
            params={"user_id": f"eq.{user_id}", "platform": "eq.discord", "select": "id"},
        )
